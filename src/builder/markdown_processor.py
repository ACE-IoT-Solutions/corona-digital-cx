...
class MarkdownProcessor:
    ...
    def remove_yaml_frontmatter(self, content: str) -> str:
        ...
        """
        # Pattern to match YAML frontmatter at the start of the file
    ...
    def combine_files(self, files: List[Path]) -> str:
        ...
        for file_path in files:
            ...
            # Remove YAML frontmatter (from frontmatter files)
            content = self.remove_yaml_frontmatter(content)

            # Skip frontmatter document metadata entirely - it's only for version tracking
            # The cover page will be generated from config instead
            if "00-frontmatter" in str(file_path) and "document-metadata" in str(file_path):
                continue
...
"""
    ...
    def build_complete_document(self) -> Tuple[str, str]:
        ...
        # Note: Metadata header removed - cover page is generated by PDF generator instead
        # This prevents duplicate metadata being rendered as text
